...
1. 지금 집중할 일을 제목으로 선언합니다. 비워 둬도 됩니다.
2. 원을 끌거나 숫자를 입력해 시간을 정합니다(1～60분).
3. 운동이나 명상처럼 두 구간을 번갈아 반복하려면 구간 반복을 켜고 각 구간 길이를 정합니다.
4. **시작**을 누르면 세션이 시작됩니다. 시작·구간 전환·종료는 소리로 알리므로 화면을 보지 않아도 됩니다.
5. 세션 중에는 남은 시간이 원형으로 표시되고, 숫자 시계는 눈 아이콘으로 보이거나 숨길 수 있습니다. **일시정지**로 멈췄다가 **이어서**로 재개하거나, **그만두기**로 세션을 끝낼 수 있습니다.
6. 세션이 끝나면 소리와 함께 화면이 알려 줍니다. 같은 설정으로 다시 시작할 수 있습니다.

화면 우측 상단에서 라이트·다크·시스템 테마와 소리 음소거를 전환할 수 있습니다.
